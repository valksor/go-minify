...
func processBundle(minifier *minify.M, bundle Bundle, outputDir string) error {
	...
	// Expand glob patterns to find all matching files
	for _, file := range bundle.Files {
		matches, err := filepath.Glob(file)
		if err != nil {
			return fmt.Errorf("failed to glob pattern %s: %w", file, err)
		}

		if len(matches) == 0 {
			return fmt.Errorf("no files found for pattern: %s", file)
		}
	...
	if err := os.WriteFile(outputPath, []byte(minified), 0600); err != nil {
...
func CleanOldBundles(bundleName, bundlesFile, outputDir string) error {
	...
	pattern := filepath.Join(outputDir, fmt.Sprintf("%s.*.min.js", bundleName))
...
func AndVersionFile(inputPath, outputDir string, fileType string) (string, error) {
	...
	if err := os.WriteFile(outputPath, minified, 0600); err != nil {
